...
const NREL_ENDPOINT = 'https://developer.nlr.gov/api/alt-fuel-stations/v1/nearest.json';
...
async function getGeoFromIP(){
  ...
  try {
    ...
    const infoRes = await fetch(`https://ipinfo.io/${ip}/json?token=f206ae74bbc8f9`);
...
async function searchUsingIP(){
  ...
  if(!geo){
    setStatus('IP failed — using default ZIP (80302)…');
    await queryNrelByZip('80302'); // fallback
    return;
  }
...
findBtn.addEventListener('click', (e) => {
  ...
  // ZIP = only numbers
  if (/^\d+$/.test(input)) {
    queryNrelByZip(input);
  } else {
    queryNrelByLocation(input);
  }
});
